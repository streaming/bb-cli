...
type buildDefinitionParserV03 struct {
	limits ParserLimits
}

func newBuildDefinitionParserV03(limits ParserLimits) *buildDefinitionParserV03 {
	return &buildDefinitionParserV03{
		limits: limits,
	}
}

// Parse parses a build definition of this specific version.
func (s *buildDefinitionParserV03) Parse(topLevelElement map[string]interface{}) (*models.BuildDefinition, error) {
...
func (s *buildDefinitionParserV03) parseJobs(raw []interface{}) ([]models.JobDefinition, error) {
...
func (s *buildDefinitionParserV03) parseJob(raw map[string]interface{}) (*models.JobDefinition, error) {

	job := &models.JobDefinition{}

	rName, ok := raw["name"]
	if ok {
		workflow, name, err := s.parseJobName(rName)
		if err != nil {
			return nil, errors.Wrap(err, "Unable to parse job 'name' field")
		}
		job.Workflow = workflow
		job.Name = name
	}

	rWorkflow, ok := raw["workflow"]
	if ok {
		workflow, ok := rWorkflow.(string)
		if !ok {
			return nil, errors.Errorf("Expected job 'workflow' field to be a string but found: %T", rWorkflow)
		}
		if job.Workflow != "" && workflow != "" {
			return nil, errors.Errorf("Job workflow is specified in both 'workflow' and 'name' fields")
		}
		job.Workflow = models.ResourceName(workflow)
	}
...
func (s *buildDefinitionParserV03) parseStep(job *models.JobDefinition, raw map[string]interface{}) (*models.StepDefinition, error) {
...
// parseJobName parses a job's name field, to extract an optional workflow name as well as the job name.
func (s *buildDefinitionParserV03) parseJobName(raw interface{}) (workflow models.ResourceName, jobName models.ResourceName, err error) {
	str, ok := raw.(string)
	if !ok {
		return "", "", errors.Errorf("Expected job 'name' field to be a string but found: %T", raw)
	}

	match := jobNameRegex.FindStringSubmatch(str)
	if match != nil {
		workflow = models.ResourceName(match[1])
		jobName = models.ResourceName(match[2])
	} else {
		return "", "", errors.Errorf("Unable to parse %q to a fully-qualified job name", str)
	}

	return workflow, jobName, nil
}

// parseJobDependencies parses a job's dependencies to a structured list of other jobs and job artifacts
// that this job depends on.
func (s *buildDefinitionParserV03) parseJobDependencies(raw interface{}) ([]*models.JobDependency, error) {
	...
	recordJobDependency := func(workflow models.ResourceName, jobName models.ResourceName, artifactDeps ...*models.ArtifactDependency) {
		if existing, ok := jobDependenciesByJobName[jobName]; ok {
			...
			jobDependenciesByJobName[jobName] = models.NewJobDependency(workflow, jobName, artifactDeps...)
		}
	}
	...
	for _, rValue := range rawArr {
		...
		match := jobDependsOnOneArtifactFromJobRegex.FindStringSubmatch(value)
		if match != nil {
			workflow := models.ResourceName(match[1])
			jobName := models.ResourceName(match[2])
			artifactName := models.ResourceName(match[3])
			artifactDep := models.NewArtifactDependency(workflow, jobName, artifactName)
			recordJobDependency(workflow, jobName, artifactDep)
			continue
		}

		match = jobDependsOnAllArtifactsFromJobRegex.FindStringSubmatch(value)
		if match != nil {
			workflow := models.ResourceName(match[1])
			jobName := models.ResourceName(match[2])
			artifactDep := models.NewArtifactDependency(workflow, jobName, "")
			recordJobDependency(workflow, jobName, artifactDep)
			continue
		}

		match = jobDependsOnAllArtifactsFromJobShorthandRegex.FindStringSubmatch(value)
		if match != nil {
			workflow := models.ResourceName(match[1])
			jobName := models.ResourceName(match[2])
			artifactDep := models.NewArtifactDependency(workflow, jobName, "")
		...
		match = jobDependsOnJobRegex.FindStringSubmatch(value)
		if match != nil {
			workflow := models.ResourceName(match[1])
			jobName := models.ResourceName(match[2])
			recordJobDependency(workflow, jobName)
			continue
		}

		match = jobDependsOnJobShorthandRegex.FindStringSubmatch(value)
		if match != nil {
			workflow := models.ResourceName(match[1])
			jobName := models.ResourceName(match[2])
			recordJobDependency(workflow, jobName)
...
func (s *buildDefinitionParserV03) parseStepDependencies(job *models.JobDefinition, raw map[string]interface{}) ([]*models.StepDependency, error) {
	var rawArr []interface{}
	rDepends, ok := raw["depends"]
	if ok {
...
func (s *buildDefinitionParserV03) parseArtifactDefinitionStrings(raw []interface{}) ([]*models.ArtifactDefinition, error) {
...
func (s *buildDefinitionParserV03) parseArtifactDefinitionObjects(raw []interface{}) ([]*models.ArtifactDefinition, error) {
...
func (s *buildDefinitionParserV03) parseService(raw map[string]interface{}) (*models.Service, error) {
...
func (s *buildDefinitionParserV03) parseEnvironment(raw interface{}) ([]*models.EnvVar, error) {
...
func (s *buildDefinitionParserV03) parseCommands(raw []interface{}) (models.Commands, error) {
...
func (s *buildDefinitionParserV03) parseLabels(raw []interface{}) (models.Labels, error) {
...
func (s *buildDefinitionParserV03) parseStringArray(raw []interface{}) ([]string, error) {
	var strs []string
	for _, rawStr := range raw {
		switch value := rawStr.(type) {
		case string:
...
func (s *buildDefinitionParserV03) parseSecretString(rValue interface{}) (*models.SecretString, error) {
	switch value := rValue.(type) {
	case string:
...
func (s *buildDefinitionParserV03) parseDockerAuthOrNil(docker map[string]interface{}) (*models.DockerAuth, error) {
...
func (s *buildDefinitionParserV03) dedupeStringSlice(list []string) []string {
